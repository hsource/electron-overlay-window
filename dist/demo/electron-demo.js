...
function createWindow() {
  ...
  `);
    ...
    makeDemoInteractive();
    __1.OverlayController.attachByTitle(window, process.platform === 'darwin' ? 'Untitled' : 'Untitled - Notepad', { hasTitleBarOnMac: true });
}
function makeDemoInteractive() {
    ...
    function toggleOverlayState() {
        if (isInteractable) {
            ...
            __1.OverlayController.focusTarget();
            window.webContents.send('focus-change', false);
        }
        else {
            isInteractable = true;
            __1.OverlayController.activateOverlay();
